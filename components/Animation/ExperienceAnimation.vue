<template>
  <div class="relative">
      ...
      <div class="experience-wrapper relative flex flex-col gap-16 place-self-center px-4py-16 sm:py-32">
          <div 
              v-for="(experience, index) in experiencesList" 
              :key="index" 
              :class="[
                  'experience flex flex-col sm:flex-row items-center sm:items-start gap-6 sm:min-h-[300px]',
                  index % 2 === 0 ? 'sm:translate-x-[8%]' : 'sm:translate-x-[52%]'
              ]"
          >
              <div 
                  class="content flex flex-col sm:flex-row gap-4 sm:w-[70%] lg:w-[40%] p-4 bg-gray-200 rounded-lg opacity-0 transform transition-transform duration-300 border-2 border-primary"
                  :class="colorMode.value === 'dark' ? 'bg-primary-950' : 'bg-primary-50'"
              >
                  <!-- Logo de l'entreprise -->
                  <div class="basis-1/4 sm:basis-1/5">
                      <img class="w-20 sm:w-24 lg:w-28" :src="experience.logo_company" alt="Logo entreprise" />
                  </div>
                  <!-- Contenu texte -->
                  <div class="basis-3/4 sm:basis-4/5 flex flex-col gap-4">
                      <div>
                          <h3 class="text-2xl sm:text-3xl font-bold uppercase mb-2">{{ experience.title }}</h3>
                          <div class="inline-flex font-medium rounded-md text-xs sm:text-sm px-2 py-1 ring-1 ring-inset ring-gray-300 dark:ring-gray-700 text-gray-700 dark:text-gray-200 bg-gray-50 dark:bg-gray-800">
                              {{ experience.date }}
                          </div>
                      </div>
                      <p class="mt-2 text-sm sm:text-base text-pretty px-2">{{ experience.description }}</p>
                      <!-- Technologies utilisées -->
                      <div class="flex flex-col gap-4">
                          <div class="text-base sm:text-lg font-bold">Technologies utilisées</div>
                          <UCarousel v-slot="{ item }" :items="experience.logos">
                              <img :src="item" class="w-10 sm:w-12 lg:w-16 mx-2" draggable="false" />
                          </UCarousel>
                      </div>
                  </div>
              </div>
...
onMounted(() => {
  ...
  $gsap.to(line.value, {
    scrollTrigger: {
      ...
      scrub: true,
      markers: true, // Affiche les points de déclenchement
    },
    ...
    $gsap.fromTo(
      ...
      },
      {
        ...
        scrollTrigger: {
          ...
          scrub: true,
          markers: true, // Affiche les markers ici aussi
